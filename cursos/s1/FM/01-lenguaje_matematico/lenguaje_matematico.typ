...
- Números: $0,1,pi,e$
- Símbolos especiales: $emptyset$ (conjunto vació), $NN$ (números
  naturales)
...
- En $(x + y) dot z$ los paréntesis indican que primero se suma $x + y$ y
...
- Negación $not$ ("no p")
- Conjunción $and$ ("p y q")
- Disyunción $or$ ("p o q")
- condicional (o implicación) $==>$ ("p entonces q", "p implica q")
- Bi conditional (o doble implicación) $<==>$ ("p si y solo si q")
...
- Aritméticas: suma ($+$), resta ($-$), multiplicación ($times$,$dot$),
  division ($div$)
- Conjuntos: union ($union$), intersección ($inter$), diferencia ($-$)
